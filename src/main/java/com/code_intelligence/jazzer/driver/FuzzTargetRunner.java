...
public final class FuzzTargetRunner {
    ...
    private static class DumpTrigger extends Throwable {
        private static final AtomicInteger counter = new AtomicInteger(0);
        private final int hash;

        private DumpTrigger(int hash) {
            this.hash = hash;
            this.setStackTrace(new StackTraceElement[]{
                    new StackTraceElement("a", "b", "c", hash)
            });
        }

        public static DumpTrigger next() {
            return new DumpTrigger(counter.getAndIncrement());
        }
